...
# ============================================================================
# Advanced primitives (from dsl_to_onnx.py)
# ============================================================================

def _mostcolor(x: torch.Tensor) -> torch.Tensor:
    """Most frequent non-background color → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    counts = []
    ...
    counts[0] = -1
    best = counts.argmax()
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    out[0, best, 0, 0] = 1.0
    return out


def _leastcolor(x: torch.Tensor) -> torch.Tensor:
    """Least frequent non-background color → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    counts = []
    for c in range(NUM_COLORS):
        if c == 0:
...
def _palette(x: torch.Tensor) -> torch.Tensor:
    """Set of colors present → (1,10,1,1) one-hot mask."""
    d = oh_decode(x)
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    for c in range(NUM_COLORS):
        if (d == c).sum() > 0:
...
def _numcolors(x: torch.Tensor) -> torch.Tensor:
    """Number of unique colors → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    n = 0
    for c in range(NUM_COLORS):
        if (d == c).sum() > 0:
            n += 1
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    out[0, min(n, 9), 0, 0] = 1.0
    return out


def _height(x: torch.Tensor) -> torch.Tensor:
    """Height of content bounding box → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    is_content = (d > 0).float()
    row_has = is_content.sum(dim=3).squeeze()
    h = row_has.sum().clamp(min=1)
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    out[0, min(int(h.item()), 9), 0, 0] = 1.0
    return out


def _width(x: torch.Tensor) -> torch.Tensor:
    """Width of content bounding box → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    is_content = (d > 0).float()
    col_has = is_content.sum(dim=2).squeeze()
    w = col_has.sum().clamp(min=1)
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    out[0, min(int(w.item()), 9), 0, 0] = 1.0
    return out


def _portrait(x: torch.Tensor) -> torch.Tensor:
    """True if height > width → (1,10,1,1) one-hot."""
    d = oh_decode(x)
    is_content = (d > 0).float()
    row_has = is_content.sum(dim=3).squeeze()
    col_has = is_content.sum(dim=2).squeeze()
    h = row_has.sum()
    w = col_has.sum()
    val = 1.0 if h > w else 0.0
    out = torch.zeros(1, NUM_COLORS, 1, 1, device=x.device)
    out[0, int(val), 0, 0] = 1.0
    return out
...
def _objects(x: torch.Tensor, univalued: bool = True,
             diagonal: bool = False, without_bg: bool = True) -> torch.Tensor:
    """Connected components → merged mask of all non-background content.

    Uses iterative min-label propagation for connected components.
    """
    d = oh_decode(x)
    H, W = x.shape[2], x.shape[3]
    ...
    # Initialize labels: each pixel gets its flat index
    flat_idx = torch.arange(H * W, dtype=torch.float32, device=x.device)
    labels = flat_idx.view(1, 1, H, W).expand_as(mask) * mask

    # Propagate labels via min
    for _ in range(max(H, W)):
        ...
        for dr, dc in [(0, 0), (0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            new_labels = torch.where(
                (shifted < new_labels) & (shifted > 0) & (mask > 0),
                shifted,
                new_labels
            )
        labels = new_labels
...
def _colorfilter(x: torch.Tensor, color: int) -> torch.Tensor:
    """Filter: keep only pixels of given color. Returns (1,10,H,W)."""
    d = oh_decode(x)
    match = (d == color).float()
    out = torch.zeros_like(x)
    out[:, color] = match.squeeze(1)
    return out


def _sfilter(x: torch.Tensor, pred_fn=None) -> torch.Tensor:
    """Filter grid: keep only matching pixels (identity fallback)."""
    return x


def _mfilter(x: torch.Tensor, pred_fn=None) -> torch.Tensor:
    """Filter then merge. Returns merged mask."""
    return x


def _compose(x: torch.Tensor, f=None, g=None) -> torch.Tensor:
    """Sequential: f(g(x)). Handled by compiler expansion."""
    return x


def _fork(x: torch.Tensor, f=None, g=None) -> torch.Tensor:
    """Parallel: merge(f(x), g(x)). Handled by compiler expansion."""
    return x


def _rbind(x: torch.Tensor, fn_name="", arg=None) -> torch.Tensor:
    """Partial application: fn(x, arg). Handled by compiler expansion."""
    return x


def _lbind(x: torch.Tensor, fn_name="", arg=None) -> torch.Tensor:
    """Partial application: fn(arg, x). Handled by compiler expansion."""
    return x


def _branch(x: torch.Tensor, cond=None, f=None, g=None) -> torch.Tensor:
    """Conditional: cond ? f(x) : g(x). Handled by compiler expansion."""
    return x


def _power(x: torch.Tensor, fn=None, n: int = 1) -> torch.Tensor:
    """Iterate: fn^n(x). Handled by compiler expansion."""
    return x
...
def fill_grid(x: torch.Tensor, color: int, mask: torch.Tensor) -> torch.Tensor:
    """Fill positions where mask has any non-zero channel with given color.

    DSL signature: fill(grid, value, patch)
    mask is a one-hot tensor from ofcolor/mapply/mfilter.
    Uses any-channel detection to handle both ofcolor (channel 1) and
    merged object grids (any channel) as fill masks.
    """
    m = (mask.sum(dim=1, keepdim=True) > 0).float()  # (1,1,H,W) any content
    bg = x * (1 - m)
...
def underfill_grid(x: torch.Tensor, color: int, mask: torch.Tensor) -> torch.Tensor:
    ...
    m = (mask.sum(dim=1, keepdim=True) > 0).float()
    d = oh_decode(x)
    is_bg = (d == 0).float()
    fill_mask = m * is_bg
    bg = x * (1 - fill_mask)
...
def cover_grid(x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
    """Erase positions where mask has any content (set to background color 0)."""
    m = (mask.sum(dim=1, keepdim=True) > 0).float()
    return x * (1 - m)
...
def paint_grid(x: torch.Tensor, obj_mask: torch.Tensor) -> torch.Tensor:
    ...
    is_obj = (obj_mask.sum(dim=1, keepdim=True) > 0).float()
    bg = x * (1 - is_obj)
...
PRIM_DISPATCH = {
    ...
    "canvas": canvas_grid,
    # Advanced: color queries
    "mostcolor": _mostcolor, "leastcolor": _leastcolor,
    "palette": _palette, "numcolors": _numcolors,
    # Advanced: shape queries
    "height": _height, "width": _width,
    "portrait": _portrait, "square": _square,
    # Advanced: objects (connected components)
    "objects": _objects,
    # Advanced: higher-order (inlined)
