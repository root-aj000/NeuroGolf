...
def try_cnn_train(task_num: int, meta: Dict, output_dir: Path,
    ...
    # Advanced training
    from train_advanced import train_advanced, export_to_onnx, count_params

    model, acc = train_advanced(task_num, X, Y, max_epochs=max_epochs,
                                lr=lr, verbose=True)

    if model is None or acc < 1.0:
    ...
    print(f"    params={params:,}  size={Path(onnx_path).stat().st_size/1024:.1f}KB")
